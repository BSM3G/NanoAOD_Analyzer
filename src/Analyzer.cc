...
void Analyzer::getGoodParticles(int syst){
  ...
  getPassingPUJetIDJets(CUTS::eRJet1, _Jet->pstats["Jet1"],syst); //12.16.20
...
void Analyzer::getFailingPUJetIDJets(CUTS ePos, const PartStats& stats, const int syst) {
...
void Analyzer::getPassingPUJetIDJets(CUTS ePos, const PartStats& stats, const int syst) {
...
void Analyzer::fill_histogram(std::string year) {
  ...
  if(!isData){
    ...
    if(distats["Run"].bfind("ApplyISRZBoostSF") && isVSample){
      ...
      //std::cout << "VBFSUSYZBoostSF has been applied" << std::endl;
    }
